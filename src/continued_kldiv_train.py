...
student_model_id = "../models/new_kldiv/model_kldiv_continued"
optimizer_path = "../optimizers/new_optim_kldiv.pt"
...
for batch in train_dataloader: #tqdm(dataloader, desc="Processing batches", file=sys.stdout):
    ...
    if iteration % 500 == 0:
        ...
        log_file.write(f"[Validation Loss] Hard: {avg_hard_val:.4f}, Soft: {avg_soft_val:.4f}\n")
        log_file.flush()
        ...
        if (iteration - iteration_begin) == 14000:
            ...
            torch.save(optimizer.state_dict(), optimizer_path[:-3] + f"_{iteration}.pt")
            log_file.close()
            with open("../still_training_kldiv.flag", "w") as f:
                f.write("continue")
            break
